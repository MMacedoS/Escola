...
$verifica_busca="SELECT * FROM avaliacao_prova where id_disciplina='$dis' and professor='$code' and id_curso='$curso' and bimestre='$bimestre' and ano_letivo='$ano_letivo'";
$con_verifica=mysqli_query($conexao,$verifica_busca);
if(mysqli_num_rows($con_verifica)==0){			
$sql_3 = "INSERT INTO avaliacao_prova (data, status, professor,id_curso, id_disciplina, detalhes, bimestre,ano_letivo, data_aplicacao)
...
echo "<script language='javascript'>window.alert('atividade cadastrada com sucesso! Click em OK para cadastrar outras!');window.location='cadastrar_provas.php?tipo=provas&code=$code&selec=$selec';</script>";

die;		
}//fim if verifica
else{
echo "<script language='javascript'>window.alert('atividade ja existe! Click em OK para cadastrar outra!');window.location='cadastrar_provas.php?tipo=provas&code=$code&selec=$selec';</script>";
}
}}?>
