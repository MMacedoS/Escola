...
$verifica_busca="SELECT * FROM avaliacao_teste where id_disciplina='$dis' and professor='$code' and id_curso='$curso' and bimestre='$bimestre' and ano_letivo='$ano_letivo'";
$con_verifica=mysqli_query($conexao,$verifica_busca);
if(mysqli_num_rows($con_verifica)==0){	
$sql_3 = "INSERT INTO avaliacao_teste (data, status, professor,id_curso, id_disciplina, detalhes, bimestre,ano_letivo, data_aplicacao)
 VALUES ('$date', 'Ativo', '$code', '$curso', '$dis', '$detalhes', '$bimestre', '$ano_letivo','$aplicacao')";
mysqli_query($conexao, $sql_3);
		
$sql_4 = "INSERT INTO mural_aluno (date, status, curso, titulo,origem) VALUES ('$date', 'Ativo', '$curso', 'As notas das Avaliações Teste estão sendo divulgadas','Avaliação Teste')";
mysqli_query($conexao, $sql_4);

echo "<script language='javascript'>window.alert('atividade cadastrada com sucesso! Click em OK para cadastrar outras!');window.location='cadastrar_teste.php?tipo=teste&code=$code&selec=$selec';</script>";

die;		
}//fim if verifica
else{
echo "<script language='javascript'>window.alert('atividade ja existe! Click em OK para cadastrar outra!');window.location='cadastrar_teste.php?tipo=teste&code=$code&selec=$selec';</script>";
}
}}?>
...
<div class="row">
   ...
   <select name="bimestre" size="1">
    <?php 
    $d=@$_POST['dis'];
    $buscaUnidade="SELECT * FROM unidades where unidade not in (select bimestre from avaliacao_teste where id_disciplina='$d' )";
