...
<link rel="stylesheet" type="text/css" href="css/fazer_rapida.css"/>
...
$sql_1 = "SELECT e.code, e.nome FROM estudantes e WHERE e.serie ='$curso' and e.turno='$turno' order by e.nome asc";
...
<?php 
 ...
 while($res_1 = mysqli_fetch_assoc($resultado)){
   ...
   $nome_aluno= $res_1['nome'];
   $sql_select_falta="SELECT presente FROM `chamadas_em_sala` where code_aluno='$code_aluno' and disciplina='$dis'";
   $con_select=mysqli_query($conexao,$sql_select_falta);
   $con_qtde_select=mysqli_num_rows($con_select);
   while ($res_select=mysqli_fetch_assoc($con_select)){
   $situacao=$res_select['presente'];
   if ($con_qtde_select!='') {
  ...
  <tr>
    ...
    ?>      
      ...
      <label for="radio">SIM </label> 
       ...
       <td width="62"><button id="btn_alter" name="inserir" value="Guardar"><img  border="0" src="../image/deleta.png" width="22" /></td>
         <?php }
