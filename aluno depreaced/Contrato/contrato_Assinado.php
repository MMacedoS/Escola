...
$result=$pdo->query("select c.*, e.nome  from contrato c inner join estudantes e on c.id_estudantes=e.id_estudantes where contrato_id='$id_contrato'");
 $result=$result->fetchAll(PDO::FETCH_ASSOC);
	foreach($result as $key=>$r_aluno){
	$nome = $r_aluno['nome'];
    $ano = $r_aluno['ano'];
    $nome_um = $r_aluno['nome_um'];
    $rg_um = $r_aluno['rg_um'];
    $cpf_um = $r_aluno['cpf_um'];
    $endereco_um= $r_aluno['endereco_um'];
    $nome_dois = $r_aluno['nome_dois'];
    $rg_dois = $r_aluno['rg_dois'];
    $cpf_dois = $r_aluno['cpf_dois'];
    $endereco_dois = $r_aluno['endereco_dois'];
    $serie = $r_aluno['ano_serie'];
    $turno = $r_aluno['turno'];
    $img_um = $r_aluno['img_um'];
    $img_dois = $r_aluno['img_dois'];
    $ordem=$r_aluno['ordem'];
    ...
    <page size="A4">
        <form method="POST" enctype="multipart/form-data" action="contrato_cliente.php">
            ...
            <div class="container">
                ...
                <div class="paragraph">
                    <b>CONTRATANTE(S): </b><u><?=$nome_um?></u>,  portador(a) da Carteira de Identidade n.º	<u><?=$rg_um?></u>,   e   inscrito   no   CPF/MF sob n.º    <u><?=$cpf_um?></u>

 residente e domiciliado(a) à <u><?=$endereco_um?></u> e <u><?=$nome_dois?></u>, portador(a) da Carteira de Identidade n.º	<u><?=$rg_dois?></u>, inscrita no CPF/MF sob   n.º <u><?=$cpf_dois?></u>, residente e domiciliado(a) à <u><?=$endereco_dois?></u>
responsável(eis) pelo(a) aluno(a):</b>  <u><?=$nome?></u>
regularmente matriculado(a) no(a)	<u><?=$serie?></u>, no turno	<u><?=$turno?></u>.
 </div>
 <div class="paragraph">
                    ...
                    <div class="ass">
                        <div class="assinaturas">
                            <div id="container_ass_um" class="assinatura margin-right">
                                ...
                                <img src="<?=$img_um?>.jpg" alt="Assinatura" width="100%" height="100%" >
                            ...
                            <div id="container_ass_dois" class="assinatura margin-left">
                                  <img src="<?=$img_dois?>.jpg" alt="Assinatura" width="100%" height="100%" >
                            </div>
