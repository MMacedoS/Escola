...
<br /><a class="a3" rel="stylesheet" href="todas_pro_trans.php?pg=projetos-transversal&selec=<?php echo $selec; ?>&code=<?php echo $code;?>">Atualizar Pagina</a>
</div>
...
<h1>Turmas:
 <select name="busca" id="cooler">  <!--  Função para recarregar a página com o grupo escolhido  -->
            ...
            <?php
             $sql_select="SELECT DISTINCT c.id_cursos,c.curso FROM projetos_transversal ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where professor='$code' and cat.id_categoria='$selec' and ati.ano_letivo=2020 ORDER BY ati.id_pro_transversal DESC";
            $result1=mysqli_query($conexao,$sql_select);   
...
$(document).ready(function(){
 ...
 <h1>Abaixo, segue o histórico bimestral de <strong> Projeto Transversal</strong> de suas turmas!</h1>
...
if(isset($_GET['busca'])){
  $res=$_GET['busca'];
  $sql_1  = "SELECT ati.*, cat.categoria FROM projetos_transversal ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where professor='$code' and cat.id_categoria='$ensino' and ati.id_curso='$res' and ano_letivo=2020 ORDER BY id_pro_transversal DESC";

}else{
  $sql_1  = "SELECT ati.*, cat.categoria FROM projetos_transversal ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where professor='$code' and cat.id_categoria='$ensino' and ano_letivo=2020 ORDER BY id_pro_transversal DESC";
...
echo "<script language='javascript'>window.location='todas_pro_trans.php?pg=projetos-transversal&selec=$selec'';</script>";
