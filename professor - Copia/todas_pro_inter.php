...
<!-- <br /><a class="a2" rel="superbox[iframe][350x400]" href="cadastrar_pro_inter.php?tipo=projetos_interdisciplinar&selec=<php //echo $selec;?>&code=<php //echo $id_professor; ?>">Cadastrar Atividade</a> -->
<br /><a class="a3" rel="stylesheet" href="todas_pro_inter.php?pg=projetos-interdisciplinar&selec=<?php echo $selec; ?>&code=<?php echo $code; ?>">Atualizar Pagina</a>
</div>
...
<h1>Turmas:
 <select name="busca" id="cooler">  <!--  Função para recarregar a página com o grupo escolhido  -->
            ...
            <?php
             $sql_select="SELECT DISTINCT c.id_cursos,c.curso FROM projetos_interdisciplinar ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where professor='$code' and cat.id_categoria='$selec' and ati.ano_letivo=2020 ORDER BY ati.id_pro_inter DESC";
            $result1=mysqli_query($conexao,$sql_select);   
...
$(document).ready(function(){
 ...
 <h1>Abaixo, segue o histórico bimestral dos projetos Interdisciplinares e trabalhos de suas turmas!</h1>
...
if(isset($_GET['busca'])){
  $res=$_GET['busca'];
  $sql_1  = "SELECT ati.*, cat.categoria FROM projetos_interdisciplinar ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where professor='$code' and cat.id_categoria='$ensino' and ati.id_curso='$res' and ano_letivo=2020 ORDER BY id_pro_inter DESC";

}else{
  $sql_1  = "SELECT ati.*, cat.categoria FROM projetos_interdisciplinar ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where professor='$code' and cat.id_categoria='$ensino' and ano_letivo=2020 ORDER BY id_pro_inter DESC";
}//fim if busca
 }else{
...
<table class="users" id="table-responsive" border="0">
  ...
  <tr>
    <!-- <td><a rel="superbox[iframe][350x400]" href="editar_pro_inter.php?id=<php //echo $res_1['id_pro_inter']; ?>&code=<php //echo $code; ?>&selec=<php //echo $selec;?>">Editar</a></td> -->
    <td colspan="3"><a href="correcao_pro_inter.php?pg=projetos_interdisciplinar&selec=<?php echo $_GET['selec']; ?>&id=<?php echo $res_1['id_pro_inter']; ?>">Lançar notas</a></td>
    <td></td>
