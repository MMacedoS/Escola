...
<title>Interdisciplinares</title>
...
$idnota=$_GET['idNota'];
$deleta_nota="DELETE FROM notas_pro_inter where id_notas_pro_inter='$idnota'";
$deleta_nota=mysqli_query($conexao,$deleta_nota);
if($deleta_nota){
 echo "<script language='javascript'>window.location='correcao_pro_inter.php?pg=projetos-interdisciplinar&selec=$selec&id=$id';</script>";

}//fim se deleta
}
$busca_prova="SELECT id_disciplina from projetos_interdisciplinar where id_pro_inter='$id'";
...
<h1><a class="a3" rel="stylesheet" href="correcao_pro_inter.php?pg=projetos_interdisciplinar&selec=<?php echo $selec; ?>&id=<?php echo $id; ?>">Atualizar Pagina</a></h1>
<br>
 <h1>Abaixo, segue os alunos desta disciplina:  <?php echo $disc."  ".$curso;?>.       <h1>Lançar notas dos Trabalhos Interdisciplinares: Valor maximo 1.0.</h1></h1>

 
<?php

$sql_1 = "SELECT * FROM projetos_interdisciplinar WHERE id_pro_inter = '$id'";
...
<table class="users" id="table-responsive" border="0">
  ...
  <tr>
    ...
    <?php
    $sql_4 = "SELECT * FROM notas_pro_inter WHERE code = '$code_aluno' AND bimestre = '$bimestre' and id_atividade='$id'";
...
if(($nota>1)){
 ...
 $sql_3 = "INSERT INTO notas_pro_inter (code, bimestre, id_disciplina, nota, id_atividade,ano_letivo) VALUES ('$code_aluno', '$bimestre', '$disciplina', '$nota', $id,'$date')";
