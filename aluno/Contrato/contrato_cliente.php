...
///salvando os arquivos caso seja pressionado o botão salvar
// var_dump($_POST);

if (@$_POST['acao'] == 'Salvar') {

     $matricula=$_POST['matricula'];
     $id_estudante=$_POST['id'];    
     $contrato_id=$id_estudante.$nextAno;
 ...
 // $img_um;
    ...
    //// verificar se o contrato ja existe 
   
        //code...
    try {
        //code...
   
    $verifica=$pdo->query("SELECT contrato_id FROM contrato WHERE contrato_id='$contrato_id'");
       ...
       echo $err;
        ...
        if($verifica==0){
            ...
            if (is_uploaded_file($_FILES['ass_um']['tmp_name'])) {
                $img_um="../../anexos/contrato/".$matricula."_1";
                move_uploaded_file($_FILES['ass_um']['tmp_name'],"$img_um".".jpg"); // line 21
                $img_dois="../../anexos/contrato/".$matricula."_2";
                move_uploaded_file($_FILES['ass_dois']['tmp_name'],"$img_dois".".jpg");
            }


            $inserir=$pdo->prepare("INSERT INTO contrato (contrato_id,ano, nome_um, rg_um, cpf_um, endereco_um, nome_dois, rg_dois, cpf_dois, endereco_dois, ano_serie, turno, img_um, img_dois, created_at, matricula,id_estudantes,ordem) VALUES (:contrato,:ano, :nome_um, :rg_um, :cpf_um, :endereco_um, :nome_dois,:rg_dois, :cpf_dois, :endereco_dois, :serie, :turno, :img_um, :img_dois, now(), :matricula, :estudante,:ordem)");
            $inserir->bindValue(":contrato",$contrato_id);
            $inserir->bindValue(":ano",$nextAno);
            $inserir->bindValue(":nome_um",$nome_um);
            $inserir->bindValue(":rg_um",$rg_um);
            $inserir->bindValue(":cpf_um",$cpf_um);
            $inserir->bindValue(":endereco_um",$endereco_um);
            $inserir->bindValue(":nome_dois",$nome_dois);
            $inserir->bindValue(":rg_dois",$rg_dois);
            $inserir->bindValue(":cpf_dois",$cpf_dois);
            $inserir->bindValue(":endereco_dois",$endereco_dois);
            $inserir->bindValue(":serie",$ano_serie);
            $inserir->bindValue(":turno",$turno);
            $inserir->bindValue(":img_um",$img_um);
            $inserir->bindValue(":img_dois",$img_dois);
            $inserir->bindValue(":matricula",$matricula);
            $inserir->bindValue(":estudante",$id_estudante);
            $inserir->bindValue(":ordem",$ordem);
                 
                $inserir->execute();
              if($inserir){
                $inserirMural=$pdo->query('INSERT INTO mural_tesouraria(data,status,titulo) values(now(),"assinado","Contrato assinado pela partes dos responsaveis ");');
                ...
                echo "erro inserção mural<br/>";
                
            }
             
        }else{
            echo "ERRO PDO...inserção contrato<br/>";
        } 
...
$result=$pdo->query("SELECT c.curso,c.ordem,c.id_cursos as turma,c.turno,e.nome,ce.ano_letivo FROM estudantes e 
...
INNER JOIN categoria cat on c.id_categoria=cat.id_categoria
 ...
 $result=$result->fetchAll(PDO::FETCH_ASSOC);
    ...
    if($ordem<=3){
        $contratada="Instituição Dantas  &  Miranda LTDA";
        $cnpj="22.010.445/0001-95";

    }elseif($ordem>3 && $ordem<=9){
        $contratada="Almeida & Dantas LTDA";
        $cnpj="00.340.583/0001-08";
    }else{
        $contratada="Miranda & Santos LTDA";
        $cnpj="09.578.728/0001-70";
    }
  }?>
<body>
    <page size="A4">
        <form method="POST" enctype="multipart/form-data" action="contrato_cliente.php">
            ...
            <input type="hidden" name="turma" value="<?=@$curso?>">
 ...
 residente e domiciliado(a) à <input required type="text" size="40" placeholder="Endereço" name="end_um" id="end_um"> e <input required type="text" size="50" placeholder="Nome" name="nome_dois" id="nome_dois">, portador(a) da Carteira de Identidade n.º	<input required type="text" size="14" placeholder="RG" name="rg_dois" id="rg_dois">, inscrita no CPF/MF sob   n.º <input required type="text" size="14" class="cpf" placeholder="CPF" name="cpf_dois" id="cpf_dois">, residente e domiciliado(a) à <input required type="text" size="40" placeholder="Endereço" name="end_dois" id="end_dois">
responsável(eis) pelo(a) aluno(a) <b></b>  <input required type="hidden" size="55" placeholder="Nome do Aluno" name="nome_aluno" id="nome_aluno" value="<?=$nome?>"><u><?=$nome?></u>
regularmente matriculado(a) no(a)	<input required type="hidden" size="8" placeholder="Ano/Série" name="ano_serie" id="ano_serie" value="<?=$serie?>"><u><?=$serie?></u>, no turno	<input required type="hidden" size="10" placeholder="Turno" name="turno" id="turno" value="<?=$turno?>"><u><?=$turno?></u>.
<div class="paragraph">
...
As partes contratantes, acima qualificadas, firmam o presente <b>CONTRATO DE PRESTAÇÃO DE SERVIÇOS EDUCACIONAIS</b>, regido pela legislação brasileira aplicável e pelas cláusulas e condições que seguem, ficando o(s) <b>CONTRATANTE(S)</b> ciente(s), desde logo, da obrigação da <b>CONTRATADA</b> com relação a normas e orientações especiais emanadas dos órgãos responsáveis pela educação brasileira e pela administração dos Sistemas de Ensino, as quais poderão, a qualquer tempo, alterar, suprimir ou acrescentar direitos e deveres às <b>PARTES</b>, mesmo no curso da execução do contrato.
                ...
                <div class="paragraph">
                <b>CLÁUSULA PRIMEIRA:</b> O presente <b>CONTRATO</b> tem como objeto a prestação de serviços educacionais, em favor do(a) aluno(a) beneficiario(a) indicado(a) no preâmbulo deste <b>CONTRATO</b> e no Requerimento de Matrícula, durante o ano letivo de <u><?=@$ano_letivo+1?></u>. Para tanto, a <b>CONTRATADA</b> assegura ao(s) <b>CONTRATANTE(S)</b> uma vaga no seu corpo discente, a ser utilizada pelo(a) aluno(a), beneficiário(a) deste <b>CONTRATO</b>, conforme os dados especificados no <b>Requerimento de Matrícula</b>.
                </div>
                <div class="paragraph">
...
válidas as correspondências enviadas aos endereços e e-mails constantes do presente contrato, inclusive para efeitos de citação  Judicial, podendo esta  ser  efetivada por edital.
                ...
                <div class="paragraph">
                    <b>PARÁGRAFO PRIMEIRO:</b> Fica(m) ciente(s) o(s) <b>CONTRATANTE(S)</b> que a matrícula do aluno somente
poderá ser efetivada mediante o pagamento da matrícula e a assinatura dos pais ou responsáve(is).
                </div>
                <div class="paragraph">
                ...
                <p>Crie sua assinatura por extenso abaixo, depois save e insira no campo vermelho</p>
