...
if ($_POST['acao'] == 'Salvar') {
    ...
    $contrato_id = 1;       
    $nome_um = $_POST['nome_um'];
    $rg_um = $_POST['rg_um'];
    $cpf_um = $_POST['cpf_um'];
    $endereco_um = $_POST['end_um'];
    $nome_dois = $_POST['nome_dois'];
    $rg_dois = $_POST['rg_dois'];
    $cpf_dois = $_POST['cpf_dois'];
    $endereco_dois = $_POST['end_dois'];
    ...
    $img_um="../../anexos/contrato/".$nome_um;
...
$result=$pdo->query("SELECT c.curso,c.turno,e.nome,ce.ano_letivo FROM estudantes e 
...
INNER JOIN categoria cat on c.id_categoria=cat.id_categoria
 WHERE e.matricula ='$matricula' and ce.ano_letivo='$ano'");
 $result=$result->fetchAll(PDO::FETCH_ASSOC);
	foreach($result as $key=>$r_aluno){
	$nome = $r_aluno['nome'];
	$serie = $r_aluno['curso'];
    $turno = $r_aluno['turno'];
    $ano_letivo=$r_aluno['ano_letivo'];
  }?>
<body>
    <page size="A4">
        <form method="POST" enctype="multipart/form-data" action="contrato_cliente.php">
            ...
            <div class="container">
                ...
                <div class="paragraph">
                    <b>CONTRATANTE(S): </b><input required type="text" size="50" placeholder="Nome" name="nome_um" id="nome_um">,  portador(a) da Carteira de Identidade n.º	<input required type="text" size="14" placeholder="RG" name="rg_um" id="rg_um">,   e   inscrito   no   CPF/MF sob n.º    <input required type="text" class="cpf" size="14" placeholder="CPF" name="cpf_um" id="cpf_um">

 residente e domiciliado à <input required type="text" size="40" placeholder="Endereço" name="end_um" id="end_um"> e <input required type="text" size="50" placeholder="Nome" name="nome_dois" id="nome_dois">, portador(a) da Carteira de Identidade n.º	<input required type="text" size="14" placeholder="RG" name="rg_dois" id="rg_dois">, inscrita no CPF/MF sob   n.º <input required type="text" size="14" class="cpf" placeholder="CPF" name="cpf_dois" id="cpf_dois">, residente e domiciliada à <input required type="text" size="40" placeholder="Endereço" name="end_dois" id="end_dois">
responsável(eis) pelo(a) aluno(a) <b></b>  <input required type="text" size="55" placeholder="Nome do Aluno" name="nome_aluno" id="nome_aluno" value="<?=$nome?>">
regularmente matriculado no(a)	<input required type="text" size="8" placeholder="Ano/Série" name="ano_serie" id="ano_serie" value="<?=$ano_letivo."/".$serie?>">,no turno	<input required type="text" size="10" placeholder="Turno" name="turno" id="turno" value="<?=$turno?>">.
 <br>
 <div class="paragraph">
<b>CONTRATADA:</b> <u>Almeida & Dantas Ltda ( Instituto Social de Tucano),</u> pessoa jurídica de direito privado, inscrita no CNPJ/MF sob n.º <u>00.340.585/0001-08</u>, estabelecida nesta cidade, à <u>Avenida Francisco Araújo de Souza, s/n.
...
<b>CONTRATANTE(S)</b> em bancos de dados cadastrais (SPC/SERASA) e de valer-se de firma especializada  de cobrança ara reaver os valores em aberto. 
                ...
                <div class="paragraph">
                    <b>Parágrafo Décimo Terceiro:</b> Conforme autorizado pelo disposto no Artigo 5º da Lei 9.870/1999 (Lei das Anuidades Escolares), declaram o(s) <b>CONTRATANTE(S)</b> que possui(em)conhecimento de que a instituição de ensino pode se recusar a deferir a rematrícula de alunos que porventura estejam inadimplentes no momento da  rematrícula,  inobstante a cobrança de referidos valores.
                </div>
                <div class="paragraph">
                ...
                <div class="paragraph">
                    <b>CLÁUSULA QUARTA:</b> Os valores da contraprestação acordada na cláusula terceira satisfazem, <b>exclusivamente</b>, à prestação de serviços decorrentes da carga horária constante da proposta curricular (Curso Regular) da <b>CONTRATADA</b> e de seu calendário escolar.
                </div>
                <div class="paragraph">
                ...
                <div class="paragraph">
                    <b>CLÁUSULA QUINTA:</b> A CONTRATADA, por mera liberalidade, adota o <b>“Sistema Positivo de Ensino”</b>,para as turmas da Educação Infantil até  o 2º ano do Ensino Fundamental e o <b>Sistema COC</b>, do 3º ano do Ensino Fundamental até o 3º ano do Ensino Médio, os quais estão de acordo com a sua Proposta Pedagógica, com a Lei de Diretrizes e Bases da Educação Nacional,  e sua prestação de serviços de ensino ocorrerá mediante a utilização do <b>MATERIAL DIDÁTICO/LIVRO DIDÁTICO INTEGRADO</b>, fornecido nas formas impressa e/ou digital, desenvolvidos especialmente para os alunos  das escolas conveniadas, atualizados periodicamente, constituindo, assim, um elemento essencial ,para o bom andamento do trato pedagógico do(a) aluno(a)beneficiário(a).
                </div>
                <div class="paragraph">
                ...
                <div class="paragraph">
                    <b>CLÁUSULA SEXTA:</b> Obriga(m)-se o(s) <b>CONTRATANTE(S)</b>, no ato da matrícula, a indicar e autorizar por escrito o médico, a clínica ou o hospital que preferencialmente deverá ser encaminhado(a) o(a) aluno(a), em caso de emergência.
                </div>
                <div class="paragraph">
                ...
                <div class="paragraph">
                    <b>CLÁUSULA OITAVA:</b> Ao firmar o presente contrato o(s) <b>CONTRATANTE(S)</b> declaram que tem conhecimento prévio do Regimento Escolar e das instruções específicas, submetendo-se as suas disposições, bem como das demais obrigações decorrentes da legislação aplicável à área de ensino. O Regimento Escolar e demais instruções estarão à disposição do(s) <b>CONTRATANTE(S)</b> para consulta, no endereço da <b>CONTRATADA</b>.
                </div>
                <div class="paragraph">
                    <b>CLÁUSULA NONA:</b> O(s) <b>CONTRATANTE(S)</b> assume(m) total responsabilidade quanto à veracidade das declarações prestadas, neste <b>CONTRATO</b> e no ato da matrícula,relativas à aptidão legal do(a) aluno(a) para a frequência na série/ano indicados.
                </div>
                <div class="paragraph">
                    <b>CLÁUSULA DÉCIMA:</b> O(s) <b>CONTRATANTE(S)</b> ficam cientes, desde logo, da obrigatoriedade do uso completo do uniforme escolar, assumindo inteiramente a responsabilidade por qualquer prejuízo acadêmico que o aluno venha a enfrentar em decorrência do descumprimento desta obrigação.
                </div>
                <div class="paragraph">
                    <b>CLÁUSULA DÉCIMA PRIMEIRA:</b> O(s) <b>CONTRATANTE(S)</b> compromete(m)-se  comunicar expressamente à <b>CONTRATADA</b> acerca da existência e do teor de decisões judiciais que venham alterar as condições
...
válidas as correspondências enviadas aos endereços e e-mails constantes do presente contrato, inclusive para efeitos de citação  Judicial, podendo esta  ser  efetivada por edital.
                ...
                <div class="paragraph">
                    <b>CLÁUSULA DÉCIMA QUARTA:</b> A <b>CONTRATADA</b> não se responsabilizará pela perda de material escolar de uso individual ou de pertences pessoais trazidos pelo(a) aluno(a) para o interior da escola, estejam ou não identificados,bem como equipamentos eletrônicos.
                </div>
                <div class="paragraph">
                ...
                <div class="paragraph">
                    <b>CLÁUSULA DÉCIMA QUINTA:</b> O(s) <b>CONTRATANTE(S)</b> declara(m), neste ato e sob as penas da lei:
                </div>
                <div class="paragraph">
                ...
                <div class="paragraph">
                    <b>CLÁUSULA DÉCIMA SÉTIMA:</b> A <b>CONTRATADA</b> não estará obrigada a renovar a matrícula do(a) aluno(a) beneficiário(a) deste <b>CONTRATO</b> para o período letivo posterior,caso o(s) <b>CONTRATANTE(S)</b> ou o(a)próprio (a)aluno(a) não tenha(m) cumprido rigorosamente as cláusulas do presente contrato.
                </div>
                <div class="paragraph">
                    <b>CLÁUSULA DÉCIMA OITAVA:</b> Para dirimir questões oriundas deste contrato, fica eleito o Foro da Comarca de Tucano, objeto deste <b>CONTRATO</b>.
                </div>
                <div class="paragraph">
...
poderá ser efetivada mediante o pagamento da matricula e a assinatura dos pais ou responsáve(is).
                ...
                <div class="paragraph">
                    E,por estarem <b>CONTRATANTE(S)</b> E <b>CONTRATADA</b> de acordo com todos os termos e condições do presente <b>CONTRATO</b>, assinam este instrumento em duas vias de igual teor e forma,na presença das testemunhas abaixo, para que se produzam todos os efeitos legais.
                </div>
                <div class="footer">
                    <div class="ass">
                    ...
                    <div class="ass-tes">
                        <span class="label">Testemunha</span>
                        <div class="line-serrilhada"></div>
                        <span class="label">RG</span>
                        <div class="line-serrilhada-rg"></div>
                    </div>
                    <div class="ass-tes">
