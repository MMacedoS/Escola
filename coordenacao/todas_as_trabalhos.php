...
<br />
<?php if(@$_GET['filtro']){
echo '<a class="a3" rel="stylesheet" href="todas_as_trabalhos.php?pg=trabalhos&selec='.$selec.'&turma='.$_GET['turma'].'&disciplina='.$_GET['disciplina'].'">Atualizar Pagina</a>';
}else{
  echo '<a class="a3" rel="stylesheet" href="todas_as_trabalhos.php?pg=trabalhos&selec='.$selec.'">Atualizar Pagina</a>';
}?>
...
<div class="form-group ml-2 col-sm-12 col-md-3">
    ...
    <div class="form-group ml-2 col-sm-12 col-md-3">
        ...
        <?php if(@$_GET['pg']){
          echo '<input type="hidden" name="pg" value="trabalhos">';
        }?>
         <div>
...
echo "<script language='javascript'>window.location='todas_as_trabalhos.php?pg=trabalhos&selec=$s&disciplina=$tipo&turma=$serie&filtro=1';</script>";
...
if(isset($_GET['turma'])){
  ...
  $code=base64_decode($_GET['disciplina']);
   $sql_1  = "SELECT ati.*, cat.categoria FROM avaliacao_coc ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where ati.id_disciplina='$code' and cat.id_categoria='$ensino' and ati.id_curso='$res' and ati.ano_letivo='$ano' ORDER BY ati.id_ava_coc DESC";

}else{
  $sql_1  = "SELECT ati.*, cat.categoria FROM avaliacao_coc ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where ati.id_disciplina='$code' and cat.id_categoria='$ensino' and ati.ano_letivo='$ano' ORDER BY ati.id_ava_coc DESC";
}//fim if busca
