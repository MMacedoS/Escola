...
// <!-- aqui começa tudo sobre a tabela -->
 ...
 $pagina.='<table class="table-responsive table-bordered" id="customers" width="900" >
<tr>
<td rowspan="2"  class="nome" bgcolor="#efefef"><center><img src="../image/logo.png" width="100" height="100"></img><h5>ALUNOS</h5></center></td>';

      
$pagina.=' <td bgcolor="#efefef" colspan='.$dado.' align="center"><p><h5>Planilha de Frequência:</h5>' .$bimestre.'º BIMESTRE, DISCIPLINA: '.$dados[0]['nome'].',  TURMA: '.$dados[0]['curso'].'; <h5>ANO:</h5> '.$ano.' </p>
<h5>DIA/MES</h5></td>';
...
$busca_faltas=$pdo->prepare("SELECT distinct ch.falta FROM chamadas_em_sala ch inner join estudantes e on e.matricula=ch.matricula inner join disciplinas d on d.id_disciplinas=ch.id_disciplinas where d.id_disciplinas=:id and ch.bimestre=:bimestre and ch.matricula=:matricula and ch.date_day=:data order by ch.date_day asc");
  ...
  $busca_faltas->bindValue(':data',$alun['date_day']);
    ...
    $dados2=$busca_faltas->fetchAll();
     ...
     foreach($dados2 as $res){
         ...
         if($res['falta']=='0'){
            ...
            $pagina.='<td rowspan="1" class="nome" bgcolor="#efefef"><strong>'.$res['falta'].'</strong></td>';
            $f=$f+$res['falta'];
         }
    ...
    $verif=$pdo->query("SELECT cargaHoraria_diaria FROM disciplinas where id_disciplinas=".$disc);                            
    $verif=$verif->fetch();
    $total=$verif[0]*$dado;
    $p=$total-$f;
    $p=number_format(($p*100)/$total,1);
    $f=number_format(($f*100)/$total,1);
     ...
     $pagina.=' <td rowspan="1" class="nome" bgcolor="#efefef"><strong>'.$total.'</strong></td>';
