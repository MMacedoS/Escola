...
<br />
<?php if(@$_GET['filtro']){
echo '<a class="a3" rel="stylesheet" href="todas_provas.php?pg=provas&selec='.$selec.'&turma='.$_GET['turma'].'&disciplina='.$_GET['disciplina'].'">Atualizar Pagina</a>';
}else{
  echo '<a class="a3" rel="stylesheet" href="todas_provas.php?pg=provas&selec='.$selec.'">Atualizar Pagina</a>';
}?>
...
<div class="form-group ml-2 col-sm-12 col-md-3">
                    ...
                    <select name="turma" onchange="submit()" class="form-control col-sm-12 mr-5">
                                ...
                                if(!@$_GET['turma']){
                                  echo '<option value="">Turmas</option>';
                                }
                                  if(@$_GET['turma']){
    ...
    <div class="form-group ml-2 col-sm-12 col-md-3">
        ...
        <?php if(@$_GET['pg']){
          echo '<input type="hidden" name="pg" value="provas">';
        }?>
         <div>
...
echo "<script language='javascript'>window.location='todas_provas.php?pg=provas&selec=$s&disciplina=$tipo&turma=$serie&filtro=1';</script>";
...
if(isset($_GET['turma'])){
  ...
  $sql_1  = "SELECT ati.*, cat.categoria FROM avaliacao_prova ati INNER JOIN cursos c ON c.id_cursos=ati.id_curso INNER JOIN categoria cat on cat.id_categoria=c.id_categoria where ati.id_disciplina='$code' and cat.id_categoria='$ensino' and ati.id_curso='$res' and ati.ano_letivo='$ano' ORDER BY ati.id_ava_prova DESC";
...
<?php }///if num_rows
  ...
  echo '<h2><font color="blue">Nenhuma busca!</font></h2>';
}//if filtro
}//if prova
